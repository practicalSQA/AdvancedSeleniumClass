...
Then(/^Check footer menu "([^"]*)" with items$/) do |expected_text|
  footer_menu_items = $driver.find_elements(xpath: "//div[@class='ws-group-contents pet-footer-links-container-contents pet-nav-contents']//h3[@class='ws-heading  pet-nav-heading pet-footer-header-heading']")
  actual_text = []
  footer_menu_items.each do |actual_text_object|
    actual_text << actual_text_object.text
    #expect(actual_text).to eql(expected_text)
  end
  puts actual_text
  ver = actual_text.include?(expected_text)
  puts ver
  fail("Text does not match") unless ver == true
